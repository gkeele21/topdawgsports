...
<html>
  <head>
    <title>TopDawgSports - March Madness Seed Challenge Standings</title>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $("#tblStandings").tablesorter({widgets: ['zebra']});
        });
    </script>
    <style type="text/css">
                       ...
                       <div id="weekNumLinks">                           
                           ...
                           <c:forEach items="${standingsWeeks}" var="week">
                               <a <c:if test="${week.FSSeasonWeekID == displayWeek.FSSeasonWeekID}">class="highlightedWeek"</c:if>
                                   href="seedChallengeStandings.htm?fsSeasonWeekID=${week.FSSeasonWeekID}">${week.FSSeasonWeekNo}
                               </a>
                           </c:forEach>
                               ...
                               <tr>
                                   ...
                                   <td><a title="${s.FSTeam.FSUser.firstName} ${s.FSTeam.FSUser.lastName}"
                                            <c:if test="${leagueTournament.status != 'UPCOMING'}">
                                                href="seedChallenge.htm?dtid=${s.FSTeam.FSTeamID}" 
                                            </c:if>
                                        >${s.FSTeam.teamName}
                                        </a>
                                    </td>
                                   <td>${s.totalPoints}</td>
                                   <td>${s.roundPoints}</td>
                                   <td><fmt:formatNumber value="${s.teamsLeft}" /></td>
                                   <td 
                                        <c:if test="${s.FSTeam.isAlive == false}">
                                            class="isOut"
                                        </c:if>
                                        >${s.scenario}
                                    </td>
