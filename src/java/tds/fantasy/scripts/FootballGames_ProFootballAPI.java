 ...
 * @author grant.keele
 */
public class FootballGames_ProFootballAPI implements Harnessable {

    Logger _Logger;
    ResultCode _ResultCode = ResultCode.RC_ERROR;
    String[] _Args;

    public FootballGames_ProFootballAPI() {
    ...
    public void run() {
        try {
            ...
            _Logger.info("Starting to update nfl games...");

            updateGames_MySportsFeeds();
            _Logger.info("Done.");

            _ResultCode = ResultCode.RC_SUCCESS;
        } catch (Exception e) {
            _Logger.log(Level.SEVERE, "Exception in FootballGames.run()", e);
        }
    }

    public void updateGames_MySportsFeeds() throws Exception {

        try {
            SeasonWeek seasonWeek = new SeasonWeek(608);
//            List<Players_PlayerObj> players = MySportsFeeds_ProFootballAPI.getGames();
//            MySportsFeeds_ProFootballAPI.getGames(seasonWeek);
...
//                    "WHERE pt.SportID = 1");
    ...
    public static void main(String[] args) {
        try {
            FootballGames_ProFootballAPI players = new FootballGames_ProFootballAPI();
            players.run();
        } catch (Exception e) {
            e.printStackTrace();
        }
